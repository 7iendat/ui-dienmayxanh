...
const Categories = () => {
  return (
    <div className="w-full h-auto bg-white mt-4 grid sm:grid-cols-2 md:grid-cols-6 lg:grid-cols-8 gap-2 rounded-xl shadow overflow-hidden">
      {fakeMenuData.slice(0, 15).map((cate, index) => (
        <div
          key={index}
          className=" relative flex flex-col items-center justify-center hover:bg-gray-400/40 hover:cursor-pointer transition p-2"
        >
          {cate.isHot && (
          ...
          />
          <span className="text-center text-sm">{cate.name}</span>
        </div>
      ))}
